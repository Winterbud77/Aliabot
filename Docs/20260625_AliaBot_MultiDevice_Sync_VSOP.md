...
## 3. 🔗 Git-based Dispatching (깃 기반 지시 전달) 추가 시 연동 규칙 및 주의사항

향후 모바일 AliaBot에서 내린 의사결정이나 메모를 깃허브 원격 저장소에 직접 파일 형태로 Push하여 사무실 Claude에 전달하는 **Git-based Dispatching** 기술을 결합하여 운용할 때는 아래 안전 수칙을 준수해야 동기화 충돌을 원천 차단할 수 있습니다.

### ① Docs 폴더 단방향 미러링 구조 유지 (OneDrive 하이브리드 보존)
* 현재처럼 Obsidian Vault 자체를 깃허브에 직접 업로드하지 않고, 프로젝트 로컬 폴더(C드라이브)의 `Docs/` 내 마크다운 문서들만 파워쉘 스크립트(`sync-to-obsidian.ps1`)를 통해 원드라이브 옵시디언 영역(`100 Source`)으로 복제하는 **단방향 스마트 미러링(Smart Mirroring) 이원화 구조**를 철저히 유지하십시오.
* 실행파일이나 소스 코드는 이미 `.gitignore`를 통해 걸러지고 있으므로, 이 방식을 유지하면 원드라이브의 자동 업로드 엔진과 깃허브 버전 추적 장치가 같은 영역을 동시에 덮어쓰는 동기화 락 충돌을 근본적으로 피할 수 있습니다.

### ② 임시 격리 브랜치(Isolated Branch) 및 풀(Pull) 기반 상속 규칙
* **규칙 1 (임시 브랜치 기록)**: AliaBot이 모바일 지시사항이나 기록을 깃허브에 푸시할 때는 직접 `main` 브랜치를 덮어쓰지 않고, 임시 브랜치(예: `aliabot/dispatch-inbox`)를 생성하여 타겟 파일(예: `Docs/Instruction_Inbox.md`)만 격리 커밋하도록 설계합니다.
* **규칙 2 (Pull 기반 상속)**: 사무실 컴퓨터 복귀 시 메인 프로젝트 터미널에서 `git pull` 혹은 `git merge`를 실행하여 깃허브에 적재되어 있던 모바일 의사결정 마크다운을 로컬 작업 영역으로 안전하게 흡수(Pull)합니다.
* **규칙 3 (미러링 전파)**: 로컬 드라이브에 안전하게 풀(Pull) 받아 흡수된 문서는 파워쉘 스크립트에 의해 해시 비교를 거쳐 **OneDrive 내의 Obsidian Vault로 단방향 자동 복제**됩니다. 옵시디언 볼트가 직접 깃허브에 역방향으로 커밋을 날리는 흐름은 통제되어야 충돌이 나지 않습니다.
