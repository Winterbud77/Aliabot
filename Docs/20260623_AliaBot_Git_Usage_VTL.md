...
## 4. 📊 Git Graph 시각적 계기판 및 참조 라벨 분석 (Visual Graph & Ref Labels)

Git 패널 하단의 **`Graph` (커밋 그래프)** 섹션은 현재 프로젝트 버전의 동기화 상태를 실시간으로 감독할 수 있는 계기판 역할을 수행합니다:

### 4.1 상단 상태줄(Toolbar) 기능
* **`Auto` (자동 갱신 모드)**: 소스 제어의 상태가 바뀔 때마다 사람이 새로고침을 누르지 않아도 버전 트리 그래프를 실시간 자동 리렌더링해 주는 스위치입니다.
* **`Go to Current history item` (🎯)**: 커밋이 무수히 쌓여 스크롤이 깊어졌을 때, 내 PC의 현재 위치(HEAD 커밋)로 한 번에 즉시 스크롤을 복귀시켜 주는 버튼입니다.
* **`Fetch from all remote` (☁️⬇️)**: 내 코드를 덮어쓰지 않고, 원격 서버(GitHub)에 타인이 작성한 새 커밋 장부가 등록되었는지만 인터넷을 통해 안전하게 조회 및 갱신해 주는 업데이트 확인 버튼입니다.

### 4.2 참조 라벨(Ref Tags)의 위치적 정합성
* **`master` (로컬 브랜치 포인터)**: 내 컴퓨터의 로컬 디스크에 세팅된 최신 마스터 커밋 위치를 표시합니다.
* **`origin/master` (원격 브랜치 포인터)**: 클라우드 서버(GitHub) 상의 최신 마스터 커밋 위치를 표시합니다.
* **동기화 성공의 시각적 증명**: 로컬 커밋 이후 `Sync Changes` 전송을 마치고 나면, 로컬 `master` 태그와 구름 아이콘이 달린 원격 `origin/master` 태그가 **동일한 최상단 파란 동그라미 위치에 나란히 겹치게 됩니다.** 이는 내 로컬 디스크 파일과 GitHub 서버 백업 파일이 완전히 100% 동일한 정상 정합성 상태임을 보증하는 직관적인 시각 지표입니다.
